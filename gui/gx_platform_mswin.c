...
void gx_gui_create_window_and_surface(gx_valvecasterUI *ui) {
	...
	static TCHAR szClassName[] = TEXT("gx_valvecasterUIClass");
	WNDCLASS wndclass = {0};
...
void gx_gui_destroy_main_window(gx_valvecasterUI *ui) {
	...
	UnregisterClass(TEXT("gx_valvecasterUIClass"), NULL);
}
...
void event_handler(gx_valvecasterUI *ui) {
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	gx_valvecasterUI *ui = (gx_valvecasterUI *)GetWindowLongPtr(hwnd, GWLP_USERDATA);
